                  ...
                  PinTypes type, uint32_t value)
{
    ...
    if (pin == NC){
        ...
        switch (mode) {
            case PIN_INPUT:
                hal_gpio_init_in((int)obj->pin, (hal_gpio_pull_t)type);//TODO: check the cast
                break;
            case PIN_ANALOGIC:
                // PIN_ANALOGIC means "deactivate the pin" in the LoRaMAC-Node
                hal_gpio_deinit((int)obj->pin);
                break;
            case PIN_OUTPUT:
                hal_gpio_init_out((int)obj->pin, value);
                break;
            case PIN_ALTERNATE_FCT:
                // Not used inside the LoRaWAN API-Wrapper
                assert(0);
                break;
        }
    }
