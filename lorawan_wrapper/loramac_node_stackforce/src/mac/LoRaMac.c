...
static void OnRadioTxDone( void )
{
    ...
    {
        LOG("tx done, listening for rx in %ds",RxWindow1Delay);
    ...
    {
        LOG("tx done, NOT listening for rx...");
...
static void OnRadioRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr )
{
    ...
    LOG("pkt rx type %2x",  macHdr.Bits.MType );
    ...
    switch( macHdr.Bits.MType )
    {
        case FRAME_TYPE_JOIN_ACCEPT:
            ...
            LOG("join accept mic computed / mic pkt %4x / %4x", micRx, mic);
...
static void OnRadioRxTimeout( void )
{
    LOG("rx timeout rxslot %d", RxSlot);
...
static void OnRxWindow1TimerEvent( void )
{
    ...
    LOG("rx slot1 channel %d, droffset %d, timeout %d", Channel, RxWindow1Config.DrOffset, LoRaMacParams.MaxRxWindow);
}
...
static void OnRxWindow2TimerEvent( void )
{
    ...
    if( RegionRxConfig( LoRaMacRegion, &RxWindow2Config, ( int8_t* )&McpsIndication.RxDatarate ) == true )
    {
        ...
        LOG("rx slot2 channel %d, freq %d timeout %d", Channel, RxWindow2Config.Frequency, LoRaMacParams.MaxRxWindow);
    }
}
...
LoRaMacStatus_t SendFrameOnChannel( uint8_t channel )
{
    ...
    LOG("tx send channel %d txp %d dr %d", channel, txPower,McpsConfirm.Datarate);
